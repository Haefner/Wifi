...
public class Model {
	public List<TraceEntry> generatedTraces;
	public List<AccessPoint> accessPoints;
	public List<TraceEntry> offlineTraces;
	...
	public List<TraceEntry> generateTraces(int d0, double n, double Pd0) {
		...
		for (TraceEntry traceEntry : offlineTraces) {
			...
			for (AccessPoint ap : accessPoints) {
				...
				Pd = Pd0 * 10 * Math.log10(distance / d0);
				SignalStrengthSamples sSample = new SignalStrengthSamples();
				sSample.put(ap.getAddress(), Pd);

				TraceEntry tr = new TraceEntry(traceEntry.getTimestamp(), traceEntry.getGeoPosition(), ap.getAddress(),
						sSample);
